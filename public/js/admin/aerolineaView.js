...
if (paisInput) {
    ...
    paisInput.addEventListener("input", function () {
        ...
        paisOptions.forEach((option) => {
            ...
            option.style.display = nombre.includes(texto) || codigo.includes(texto) ? "flex" : "none";
        });
    });
    ...
    paisInput.addEventListener("blur", function () {
        setTimeout(() => {
            ...
            const paisEncontrado = [...paisOptions].find((option) => {
                return option.dataset.nombre.toLowerCase() === texto || option.dataset.codigo.toLowerCase() === texto;
            });
